...
def test_ffdnet(**args):
	...
	if not args['dont_save_results']:
		np.save(out_path + 'npy/{}.npy'.format(fn),
				(outim.data.cpu().numpy()[0]).transpose(1,2,0))
		noisyimg = variable_to_cv2_image(imnoisy)
		outimg = variable_to_cv2_image(outim)
		cv2.imwrite(out_path + "jpg/{}_noisy.png".format(fn), noisyimg)
		cv2.imwrite(out_path + "jpg/{}_ffdnet.png".format(fn), outimg)
		if args['add_noise']:
 			cv2.imwrite(out_path + "jpg/noisy_diff.png", variable_to_cv2_image(diffnoise))
 			cv2.imwrite(out_path + "jpg/ffdnet_diff.png", variable_to_cv2_image(diffout))
...
if __name__ == "__main__":
	...
	if os.path.isfile(in_path) == True:
		...
		filepaths = sorted(glob.glob(in_path + '*.jpg'))
